module.exports = class UsuarioPlataforma { 
  ...
  buscarUsuario(connection, callback){
    var sql = "SELECT * FROM usuarios_da_plataforma WHERE apelido = ? AND senha = ?";

    connection.query(sql, [this.apelido, this.senha], function (err, result) {
        if (err) throw err;
        return callback(result);
    });    
  }
